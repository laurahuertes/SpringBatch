...
public class UserUpdaterWriter implements ItemWriter<UserEntity> {

    private final DataSource dataSource;
    private ExecutionContext executionContext;

    public UserUpdaterWriter(DataSource dataSource) {
    ...
    public void write(List<? extends UserEntity> list) throws Exception {
        JdbcBatchItemWriter<UserEntity> builder = new JdbcBatchItemWriterBuilder<UserEntity>()
            .beanMapped()
                .sql("UPDATE users SET username = :username, password = :password, email = :email , fullName = :fullName WHERE id = :id ")
        ...
        List<UserEntity> users = (List<UserEntity>) this.executionContext.get("usersUpdated");
        if(users == null) {
            users = userEntities;
        } else {
            users.addAll(userEntities);
        }
        this.executionContext.put("usersUpdated", users);
    }
}
